...
서로 다른 원본 사진으로 Panel 159에서 각각 한 번 생성한 8개 샘플입니다. 짧은 영문 문구를 사용했으며 가로 1536×864 좌우 균등, 세로 1152×1536 상하 균등으로 정리하고 메타데이터 정리를 검증했습니다. 실제 모델 결과이며 모든 스타일 조건을 통과했다는 뜻은 아닙니다.

관찰된 편차: 일부 피사체나 장면이 너무 크고, 일부 제목은 이미지와 결합되지 않고 분리되어 있습니다. 모델이 추가한 숫자는 사실 정보가 아닙니다.
...
문자·점·저해상도 망점으로 이미지를 만들고 실험적 비트맵 제목을 통합합니다. 작은 피사체와 넓은 여백이 핵심입니다.
...
- `design-only`: 전체 캔버스에 Panel 159의 디자인 번역만 표시하고 사진은 보이지 않는 참고 자료로 사용합니다.
...
설치 후 Agent 세션을 다시 시작하고 `$xxd-panel-159`을 호출하세요. 사용자 단위 Codex 설치에는 `--global --agent codex --yes`를 추가할 수 있습니다.

```text
/xxd-panel-159 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ko-KR
...
전체 실행 계약은 [SKILL.md](SKILL.md), 런타임 어댑터는 [영어](references/xxd-panel-159-prompt.en.md)와 [중국어](references/xxd-panel-159-prompt.zh-CN.md)를 확인하세요.
