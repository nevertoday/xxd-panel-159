...
異なる元写真から Panel 159 で各1回生成した8点です。短い英文を使用し、横1536×864・左右均等、縦1152×1536・上下均等に整え、メタデータ清掃を検証しました。実際のモデル出力を残しており、全要件への適合を保証するものではありません。

確認した偏差：被写体や残された場面が大きすぎる例、文字と図像の絡みが弱い例があります。モデルが追加した数字は事実情報ではありません。
...
文字・点・低解像度の網点で像を構成し、実験的なビットマップ見出しを画像と一体化。小さな主題と広い余白。

## 4つの出力モード

- `top-bottom`：全幅の上下2領域のみ。実写を上、デザインを下に置き、各50%。
- `left-right`：全高の左右2領域のみ。実写を左、デザインを右に置き、各50%。上下構成へ回転しません。
- `design-only`：全画面を Panel 159 のデザイン翻訳にし、写真は見えない参照にします。
...
インストール後に Agent セッションを再起動し、`$xxd-panel-159` を呼び出します。ユーザー単位の Codex には `--global --agent codex --yes` を追加できます。

```text
/xxd-panel-159 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ja-JP
...
完全な実行契約は [SKILL.md](SKILL.md)、実行アダプターは[英語](references/xxd-panel-159-prompt.en.md)／[中国語](references/xxd-panel-159-prompt.zh-CN.md)を参照してください。
