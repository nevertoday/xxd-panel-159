...
# XXD Panel 159｜ملصق تجريبي بالحروف والنقاط
...
وُلّدت هذه النماذج الثمانية بواسطة Panel 159، كل منها بمحاولة واحدة ومن صورة مصدر مختلفة مع نص إنجليزي قصير. ضُبطت الأبعاد إلى 1536×864 بتقسيم متساوٍ يميناً ويساراً و1152×1536 بتقسيم متساوٍ أعلى وأسفل، وتم التحقق من تنظيف البيانات الوصفية. هذه نتائج فعلية وليست تأكيداً لاجتياز جميع متطلبات الأسلوب.

الانحرافات الملحوظة: بعض العناصر أو المشاهد أكبر من المطلوب، وبعض العناوين منفصلة عن الصورة بدلاً من التداخل معها. الأرقام العرضية التي أضافها النموذج ليست معلومات واقعية.
...
صورة مبنية من الحروف والنقاط والشبكات منخفضة الدقة، مع عنوان نقطي تجريبي متداخل وموضوع صغير وفراغ واسع.
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 159 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-159`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-159 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-159-prompt.en.md) أو [بالصينية](references/xxd-panel-159-prompt.zh-CN.md).
