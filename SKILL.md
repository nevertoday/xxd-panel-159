---
name: xxd-panel-159
description: "Create Panel 159 raster artwork: Post-digital Bitmap Typography. Use when the user invokes xxd-panel-159 or requests this specific visual language."
---
...
Text modes are `prompt`, `exact`, and `none`. Resolve the target locale explicitly. Exact text is passed verbatim; text-free output contains no letters, numbers, logos, labels, or pseudo-text. In prompt text mode, preserve the source brief's integrated experimental bitmap title and sparse supporting information; do not demote all typography to tiny captions. Generate source-grounded copy in the resolved locale. If text-free delivery conflicts with character-built imagery, explain the conflict and resolve it before spending an image call; never silently replace the defining visual language.
...
Write final PNGs directly inside one fresh task directory under `~/Desktop/xxd/xxd-panel-159/` or the explicit output root. Use collision-safe filenames; do not create source, mode, or size subdirectories and do not generate an automatic contact sheet.
...
- `references/xxd-panel-159-prompt.zh-CN.md` and `.en.md` — delivery adapter notes
