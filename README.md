...
以下 8 张样张由 Panel 159 基于不同源照片独立单轮生成，使用简短英文文案。已归整为横版 1536×864、左右各 50%，及竖版 1152×1536、上下各 50%，并完成元数据清理。保留单次模型结果供观察，不代表全部通过风格验收。

已观察到的偏差：部分主体或保留场景偏大；部分标题仍像独立标题而非与图像咬合。个别数字为模型添加，不是事实信息。
...
字符与网点构成图像，位图实验标题与图像咬合，黑白灰加1–3个源图点睛色，小主体大留白。
...
- `design-only`：整张画布只呈现 Panel 159 的设计转译，照片只作为不可见参考。
...
安装后重新启动 Agent 会话，然后调用 `$xxd-panel-159`。也可以按需追加 `--global --agent codex --yes` 做用户级安装。

常用调用示例：

```text
/xxd-panel-159 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale zh-CN
...
完整运行契约见 [SKILL.md](SKILL.md)；运行适配器见 [英文](references/xxd-panel-159-prompt.en.md) 与 [中文](references/xxd-panel-159-prompt.zh-CN.md)。
...
Input image for this panel: `assets/inputs/input-01`

![Re-selected input](assets/inputs/input-01.jpg)
