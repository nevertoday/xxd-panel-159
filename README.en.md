...
# XXD Panel 159｜Post-digital Bitmap Typography
...
Native PNGs are retained without resizing, cropping or split correction. Landscapes are 1672×941 (approximately 16:9); portraits are 1086×1448 (3:4). Landscape boundaries are near the midpoint; the source region in these portraits occupies approximately 45.2–50.0%. These are not all certified exact 50:50 splits.

Every full-size image and thumbnail was reviewed, and metadata cleanup was verified. These are actual outputs, **not a fully accepted style showcase**. Some subjects/scenes are oversized; some titles remain separate instead of forming integrated experimental typography.
...
Build imagery from characters, dots and low-resolution raster units, not continuous blocks. Preserve ordered density hierarchy and an integrated experimental bitmap title with sparse supporting copy. Use black/white/grey with 1–3 source-derived accents, a small subject and vast whitespace. Avoid uncontrolled glitch, full-scene tracing and ordinary pasted-on titles.
...
- `design-only`: the full canvas contains only Panel 159's designed translation; the photograph remains a non-visible reference.
...
```bash
git clone https://github.com/nevertoday/xxd-panel-159.git
npx skills add https://github.com/nevertoday/xxd-panel-159 --skill xxd-panel-159
```

Restart the agent session after installation, then invoke `$xxd-panel-159`. Add `--global --agent codex --yes` when a user-level Codex installation is wanted.

Common examples:

```text
/xxd-panel-159 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale en-US
/xxd-panel-159 photo.jpg --mode left-right --size 16:9 --text prompt --locale en-US
/xxd-panel-159 photo.jpg --mode design-only --size 9:16 --text none
/xxd-panel-159 ./photos --mode design-only --size auto,3:4 --text prompt --locale ja-JP
```

See [SKILL.md](SKILL.md) for the full runtime contract and the [English](references/xxd-panel-159-prompt.en.md) or [Chinese](references/xxd-panel-159-prompt.zh-CN.md) runtime adapter.
